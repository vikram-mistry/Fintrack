...
function syncWidgetData() {
  if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.WidgetDataBridge) {
    try {
      ...
      const widgetPayload = {
        ...
        income: totalIncome,
        month: cycleLabel // Sending cycle range instead of just month name
      };

      // DEBUG ALERT: Verify calculation
      alert(`Syncing Widget:\nExp: ${totalExpense}\nInc: ${totalIncome}\nCycle: ${cycleLabel}`);

      window.Capacitor.Plugins.WidgetDataBridge.saveData({
        json: JSON.stringify(widgetPayload)
      });
    } catch (e) {
      console.error("Widget Sync Error", e);
      alert("Widget Sync Error: " + e.message); // Temporary Debug
    }
  } else {
    console.warn("WidgetDataBridge plugin not found");
    // alert("Debug: Widget Plugin Missing! Did you add files to Xcode?"); // Temporary Debug
  }
}
  ...
  if (loaded) {
    ...
    if (localRaw) {
      try {
        ...
        // Sync Data to Widget immediately on load
        syncWidgetData();
